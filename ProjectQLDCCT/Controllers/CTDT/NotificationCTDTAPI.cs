...
{
    [Route("api/ctdt/notification")]
    [ApiController]
    public class NotificationCTDTAPI : ControllerBase
    {
        private readonly QLDCContext db;
        private readonly int unixTimestamp;
        public NotificationCTDTAPI(QLDCContext _db)
        {
            db = _db;
        ...
        {
            var idprogram = await GetUserPermissionPrograming();
            var totalRecords = await db.Notifications
                .Where(x => idprogram.Contains(x.id_program ?? 0) && x.id_user == null).CountAsync();
            var GetList = await db.Notifications
                .Where(x => idprogram.Contains(x.id_program ?? 0) && x.id_user == null)
        ...
        public async Task<IActionResult> CountNotification()
        {
            ...
            var count = await db.Notifications
                .Where(x => idprogram.Contains(x.id_program ?? 0) && x.id_user == null && x.is_read == false)
                .CountAsync();
